...
router.get("/:id", async (req, res) => {
  try {
    ...
    const questions = await Question.find({ author: user._id, isActive: true }).sort({ createdAt: -1 }).limit(10).lean()

    // Get user's answers
    const answers = await Comment.find({ author: user._id, parentComment: null })
...
router.get("/leaderboard/top", async (req, res) => {
  try {
    ...
    const users = await User.find()
      .select("name username avatar reputation badges questionsCount answersCount votesReceived")
      .sort(sortField)
